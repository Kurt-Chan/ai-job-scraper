...
- **Search preferences (chips)** — before starting a search, the user can pick Employment Type (Full-time/Part-time/Contract/Freelance/Hourly, multi-select), Pay/Currency (USD/EUR/GBP/Local, multi-select), and Location (a "Worldwide Remote" chip plus a free-text field for a specific country/region). These get assembled client-side into one natural-language preferences string (e.g. `"Egypt, Part-time or Hourly work, paid in USD"`) sent to `/api/run` and folded into both `prompts/build_queries.md` (search targeting) and `prompts/analyze.md` (scoring) — added because early runs defaulted to India-heavy, full-time-only results with no way to steer them. Selections persist in `localStorage`.
